...
local function flattenCSVRow(row)
	return concat(row, ",") .. "\n"
end

function util.writeCSV(path, tbl)
	local file = io.open(path, "w")
	if tbl.header then
		file:write(flattenCSVRow(tbl.header))
	end

	for _, v in ipairs(tbl) do
		file:write(flattenCSVRow(v))
	end
...
function util.readHeaderedCSV(path)
	local tbl = {}
	local header
	...
	for line in io.lines(path) do
		...
		if not header then
			...
			insert(tbl, setmetatable(row, rowmt))
		end
	end
